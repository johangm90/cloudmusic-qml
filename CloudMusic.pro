...
UBUNTU_TRANSLATION_DOMAIN="apu.johangm90"
...
aptest.commands = QML2_IMPORT_PATH=$$OUT_PWD/backend bash $$PWD/apu/tests/autopilot/run
aptest.depends  = sub-apu sub-backend-App

unittest.target   = check
unittest.commands = /usr/bin/qmltestrunner -input $$PWD/backend/tests/unit -import $$OUT_PWD/backend
unittest.commands += && /usr/bin/qmltestrunner -input $$PWD/apu/tests/unit -import $$OUT_PWD/backend
unittest.depends  = sub-apu sub-backend-App
